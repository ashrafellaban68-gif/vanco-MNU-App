...
def create_pdf_report(age, weight, drug, crcl, ld, md, interval, soap_text):
    ...
    content = [
        Paragraph("DoseWise: Clinical PK Report", title_style),
        Spacer(1, 20),
        ...
        Spacer(1, 20),
        Paragraph(f"<b>Final Regimen:</b> LD {round(ld)}mg | MD {round(md)}mg every {interval}h", styles['Heading2']),
        Spacer(1, 20),
        Paragraph("<b>Clinical SOAP Note:</b>", styles['Heading3']),
...
# ==============================
# ⚙️ 3. Main App Execution & Scientific Logic
# ==============================
st.set_page_config(page_title="DoseWise Clinical Master", layout="wide")
set_page_style()

st.markdown('<div class="hero"><h1>💊 DoseWise Clinical Master</h1><p>Comprehensive Pharmacokinetics Support System</p></div>', unsafe_allow_html=True)
...
# --- ADVANCED PHENYTOIN PARAMETERS ---
alb, vmax, km, s_factor = 4.4, 7.0, 4.0, 0.92 # Default
...
# 📋 5 TABS INTERFACE
# ==============================
tab1, tab2, tab3, tab4, tab5 = st.tabs(["🎯 Results", "📚 Monograph", "⚖️ Clinical Decision", "📋 Case Summary", "📝 SOAP Note"])
...
with tab2:
    st.markdown('<div class="section">', unsafe_allow_html=True)
    monographs = {"Phenytoin": ["10-20 mg/L", "1000mg/d", "Gingival Hyperplasia"], "Valproic acid": ["50-100 mg/L", "60mg/kg/d", "Hepatotoxicity"], "Carbamazepine": ["4-12 mg/L", "1600mg/d", "Auto-induction"], "Levetiracetam": ["12-46 mg/L", "3000mg/d", "Behavioral Changes"]}
    st.write(f"**Target Range:** {monographs[selected_drug][0]}")
    st.write(f"**Max Daily Dose:** {monographs[selected_drug][1]}")
    st.error(f"**Common Side Effects:** {monographs[selected_drug][2]}")
    st.markdown('</div>', unsafe_allow_html=True)

with tab3:
    st.markdown('<div class="section">', unsafe_allow_html=True)
    st.subheader("⚖️ Decision Support")
    if is_obese: st.error(f"❗ Obesity: Dose based on ABW ({d_wt:.1f} kg).")
    if crcl < 50: st.warning("⚠️ Renal Impairment: Dose reduction applied.")
    if selected_drug == "Phenytoin" and alb < 4.4:
        adj_c = target / ((0.2 * alb) + 0.1)
        st.info(f"💡 Low Albumin: Adjusted Target Css: {adj_c:.1f} mg/L.")
    st.write("Plan: Monitor for efficacy and toxicity symptoms.")
    st.markdown('</div>', unsafe_allow_html=True)
...
with tab5:
    ...
    soap_txt = f"S: {age}Y {gender} on {selected_drug}.\nO: Weight {weight}kg, SCr {scr}mg/dL, CrCl {crcl:.1f}mL/min.\nA: Patient requires individualized PK dosing due to status.\nP: Administer LD {round(ld)}mg, then MD {round(md)}mg q{interval}h. Monitor serum levels."
    ...
    pdf_file = create_pdf_report(age, weight, selected_drug, crcl, ld, md, interval, soap_txt)
    st.download_button("📥 Download Official Report", pdf_file, f"DoseWise_{selected_drug}.pdf", "application/pdf")
    st.markdown('</div>', unsafe_allow_html=True)

st.markdown("<center style='color:#64748b;'>DoseWise Clinical v7.0 | MNU Faculty of Pharmacy</center>", unsafe_allow_html=True)
